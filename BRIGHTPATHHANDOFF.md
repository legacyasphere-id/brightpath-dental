...
**Two em dashes found in the OG surface itself while shipping the next fix, corrected.** The OG/Twitter `description` and the OG image `alt` text both used an em dash, breaking the standing no-em-dash rule — worth calling out specifically because the OG description is the single most-shared string on the site, since it travels through every WhatsApp forward. Recast rather than comma-swapped: description is now "Perawatan gigi modern di Bekasi dengan booking mudah lewat WhatsApp," alt text is "BrightPath Dental adalah klinik gigi modern di Bekasi." **Known, not fixed:** the base (non-OG) `title`/`description` in `layout.tsx` still has two more em dashes — lower-priority surface (browser tab and plain search snippets, not what WhatsApp reads), left alone since it wasn't in scope for this run.

**Follow-up: base `title`/`description` swept too, and a broader finding while sweeping.** `title` recast to "BrightPath Dental | Modern Dental Care in Bekasi" (a pipe here isn't a punctuation swap — it's the standard `<title>` tag convention, structurally different from joining two clauses with a dash). `description` recast into one real sentence: "BrightPath Dental lets you book appointments, chat with our AI assistant, and get transparent pricing for dental care in Bekasi."

While confirming those were the only two left, a full-repo grep for em dashes turned up several more that predate this run and were never caught by the earlier cleanup pass — **flagged, not fixed, since this sweep was scoped to the two named strings:**
- `src/lib/ai/prompts.ts` — the model's own fallback line, said verbatim to a real patient whenever the KB doesn't cover their question: `"Saya tidak memiliki informasi tersebut — silakan hubungi kami..."` (and its English equivalent). Arguably the highest-priority one on this whole list, since it's live bot output, not marketing copy.
- `src/components/marketing/Doctors.tsx` — all three doctor image `alt` texts (accessibility copy, read by screen readers).
- `src/components/chat/LeadCapture.tsx` — the in-chat lead-capture prompt shown to real visitors.
- `src/app/api/knowledge/route.ts` and `src/components/admin/KnowledgeUpload.tsx` — admin-facing, lower stakes (Yoga only), but still copy.

**Not on this list, deliberately:** the `"—"` placeholders in `LeadsTable.tsx` and the admin dashboard's stat cards (`admin/(protected)/page.tsx`) are an "empty value" table convention, not sentence-joining punctuation — a different use of the character, not a rule violation.

**Services page/KB mismatch resolved by extending the page, not trimming the KB** — Yoga's call, since the KB already carries the necessary detail. Added three service cards to `Services.tsx` (Tooth Extraction, Crowns and Bridges, Dentures), matching the existing card structure and voice exactly (title, one sentence, "Book this service →" link), every claim drawn directly from `01-services.txt`. Two knock-on effects handled rather than left inconsistent: Hero's "6 Dental Services" stat corrected to 9 (this is the same stat RUN A corrected from a wrong "15+" — now genuinely 9, not another guess), and `LeadForm.tsx`'s service dropdown extended with the same three options, so a visitor reading about crowns on the page can actually select crowns when booking instead of falling into "Other."

**RUN G, bug one — query-side fix, deterministic expansion.** The threshold fix (above) and the Indonesian rewrite (above) both looked like reasonable explanations and both turned out to be wrong: after the rewrite, `01-services.txt` still didn't retrieve, and the five chunks that did come back scored *identically* to four decimal places versus before the rewrite (`05-faq.txt` 0.4299 vs 0.4300, `04-clinic-info.txt` 0.4118 vs 0.4119, `02-pricing.txt` and `03-doctors.txt` unchanged). The rewrite changed nothing about retrieval — proof the language hypothesis was actually disconfirmed, not just unproven, since a real content change produced zero measurable effect.

The actual mechanism, found by a controlled experiment (same KB, same minute, only phrasing varied): the failing short query ("ada layanan apa saja?") scored 0.339–0.430 across the KB; a longer, topically fuller phrasing of the identical question ("panduan layanan perawatan gigi lengkap beserta durasi dan prosedurnya") scored 0.580–0.639 across the *same* KB, and surfaced both `01-services.txt` chunks, ranked first. Every chunk's similarity moved together, not just the target document — short queries produce diffuse embeddings sitting weakly and near-equally far from everything, so ranking among topically related documents becomes close to arbitrary. This also retroactively explains the original zero-chunk bug: at the old `0.7` threshold, nothing in a short query's similarity band ever reached the bar.

Fix is query-side: `expandQueryForRetrieval()` in `retrieval.ts` wraps the user's message in a fixed, generic template before it's embedded — clinic context plus a broad vocabulary net across the KB's whole topic space (layanan, perawatan, prosedur, durasi, dokter, harga), not a guess tuned to this one failing query. No extra model call, so no added latency on a streaming reply. `retrieveContext()` embeds the expanded form; `api/chat/route.ts` still passes the raw, unexpanded `message` to `streamChat()` — only what gets searched for changes, not what the model is asked to answer.

Chosen over a HyDE-style approach (ask the model to draft a hypothetical answer, embed that) specifically because this KB is six documents about one clinic — doesn't need per-question cleverness — and a streaming chat reply can't absorb an extra model round trip for free. Confirmed correct by live verification below: HyDE doesn't earn its latency against these numbers.

**Correction, owned rather than absorbed quietly: the controlled experiment above had an uncontrolled variable.** "Panduan layanan perawatan gigi lengkap beserta durasi dan prosedurnya" wasn't just a longer version of the failing query — "durasi dan prosedurnya" echoes `01-services.txt`'s own `Durasi:` and `Proses:` fields almost word for word. Length reducing embedding diffuseness and vocabulary overlap with one specific document were confounded in that test, and it was reported as if only one variable had moved. The deterministic template above was designed with this in mind — a broad vocabulary net across the whole KB's topic space rather than generic clinic boilerplate — specifically so it doesn't depend on the confound holding. The live-verification bar (2–3 short paraphrases, not just the one known-failing query, reporting rank and similarity rather than only whether the floor was cleared) exists for the same reason: a fix that only works on the sentence already known to fail is curve-fitting, not a fix.

**Verified live: outcome one, ship it — and the confound resolves in favor of the original diffuseness theory.** Four short paraphrases against production, one of them ("bisa perawatan apa aja di sini") invented on the spot rather than pulled from any prepared list, so the template wasn't designed around it: `01-services.txt` retrieved 4/4, at 0.720 / 0.725 / 0.718 / 0.730.

Two things in the numbers beyond the pass/fail:
- **Magnitude.** The entire retrieved field moved from a 0.339–0.430 band pre-expansion to 0.665–0.789 post-expansion — every chunk's similarity improved together, not just `01-services.txt`'s. This is the evidence that settles the confound above: if the win came from lexical overlap with the services document specifically, only that document's similarity would have moved. The whole field moving is diffuseness, not vocabulary matching — the confound was real and worth raising, and the expanded test resolves it *for* the original theory, not against it.
- **Stability.** Across four different phrasings, `01-services.txt` lands in a 0.012-wide band (0.718–0.730). The template dominates the embedding enough that different surface phrasings converge on nearly the same similarity — exactly the property a stabilizing template needs. HyDE doesn't earn its added latency against these numbers. Stopping here.

Two more observations, notes rather than actions:
- Only 5 of 6 chunks return each time (`top_k=5`); the one consistently missing is `01-services.txt`'s second chunk (60 tokens, starts mid-sentence at "menggunakan teknologi CAD/CAM") — it carries the crown, bridge, and denture pricing, so those facts currently reach the model only via `02-pricing.txt`. Not urgent, but if the KB grows, this fragment should be merged into its neighbor rather than left as an orphan chunk that only ever loses the top-5 cutoff to its own sibling.
- `04-clinic-info.txt` (address, hours, contact details) now ranks first on every services question tested, around 0.785 — ahead of the services guide itself. Mildly odd given its content, not investigated further.

**Em-dash rule finished, not left half-applied.** `layout.tsx`'s base (non-OG) `title` and `description` — the two flagged and deliberately left alone in the previous pass, since it was scoped to the OG surface only — are now clean too. `title` recast to `"BrightPath Dental | Modern Dental Care in Bekasi"` (a pipe here is the standard `<title>` tag convention, not a punctuation swap for the dash). `description` recast into one real sentence: `"BrightPath Dental lets you book appointments, chat with our AI assistant, and get transparent pricing for dental care in Bekasi."`

**Final verification pass — nothing regressed.** `tsc --noEmit` clean, `eslint` clean, `npm run build` zero type errors, Vitest 29/29, Playwright 22/22 (chromium). Manual check at 375px: hero, all nine service cards, doctor row, pricing, and the chat panel — screenshotted directly (`page.locator(...).screenshot()` per section, not a full-page capture, so nothing was hidden below an unscrolled fold), all render cleanly, nothing clipped. Lighthouse against the local production build: performance 97, accessibility 90, best-practices 100, SEO 100 — accessibility findings below, logged as their own item rather than folded into this passing report.

**Known bug, flagged not fixed: `LeadForm.tsx`'s service dropdown has no linked label.** Surfaced by the Lighthouse pass above, but this is a real accessibility defect on the one form that captures patients, not just a score deduction — worth its own line rather than sitting inside a verification report. A visible "Service Needed" `<label>` sits above the `<select>`, but isn't wired to it via `htmlFor`/`id`, so a screen reader announces the control with no name. Confirmed pre-existing — only the `services` array (three new options) was touched this session, not this markup. Fix is two attributes (`id` on the `<select>`, matching `htmlFor` on the `<label>`), not attempted here since it was out of scope for a verification pass, not because it's hard.

Two related, lower-priority Lighthouse findings, same pre-existing status, recorded together since neither is a specific defect the way the above is:
- `color-contrast` (26 instances): widespread, spans nearly every section (footer, pricing, doctors, AI demo, lead form) — a site-wide pattern of muted-text utility classes (`text-clinic-muted`, `text-white/60` on the navy pricing card) not meeting 4.5:1.
- `heading-order`: one `<h4>` in `Footer.tsx` that skips a level — `Services.tsx`'s new cards use `<h3>`, not `<h4>`, unrelated to today's additions.
...
**Key migration to Supabase's new key format, and a live bug it surfaced that turned out to be unrelated to it.** Yoga migrated `SUPABASE_SERVICE_ROLE_KEY` to an `sb_secret_` key and `NEXT_PUBLIC_SUPABASE_ANON_KEY` to the publishable key, then disabled both legacy JWTs in Supabase — the only test that actually distinguishes new keys working from old keys quietly still doing the job. Verified in that disabled state: a lead row landed via the secret-key path (`createServiceClient()`) and `/api/health` returned `db: true` via the publishable-key path (`createAnonClient()`). Both paths proven live with the old format no longer available to fall back to.

Right after the migration, `/api/leads` started throwing `supabaseUrl is required` while `/api/health` and `/api/chat` kept working. First hypothesis (Yoga's) was a missing `SUPABASE_URL` variable — checked against the real Vercel env var list and wrong, no such variable was ever expected; both `createAnonClient()` and `createServiceClient()` read `NEXT_PUBLIC_SUPABASE_URL`. Second hypothesis was that `NEXT_PUBLIC_SUPABASE_URL` being marked Sensitive was blocking it from build-time inlining. Investigated rather than accepted: `createAnonClient()` and `createServiceClient()` reference the *identical* `NEXT_PUBLIC_SUPABASE_URL` expression in the same file — if inlining had eaten one, it would have eaten both, and `createAnonClient()` was demonstrably working. That ruled out a code-level explanation entirely before a fix was ever proposed. Actual cause: deployment skew, not configuration — `/api/leads` was being served by a stale build. A fresh production build fixed it, confirming the code read rather than replacing it with a different guess.

⚠️ **Real gap, independent of today's bug, worth fixing regardless:** `SUPABASE_SERVICE_ROLE_KEY` has no Preview scope in Vercel (Production only). `/api/leads` and `/api/knowledge` both still call `createServiceClient()`, so any Preview deployment hitting either route throws `supabaseKey is required.` Not touched here.
...
**Marketing UI pass — layout, spacing, and hierarchy, informed by a competitor study (damessa.id, a 24-branch chain with a branch near BrightPath's fictional location).** Brand colors, typography, and the component library untouched. Background reasoning: adopt generous whitespace and a consistent outlined line-icon set (both real strengths of the competitor site), explicitly do not adopt their tight shouting headlines, nine-item nav, or promo-driven positioning, and merchandise four things BrightPath already has that the competitor's site doesn't surface anywhere — an AI assistant (they route every question to a human who replies within an hour), published prices (they route every price question to WhatsApp), instant answers instead of a wait, and named doctors with credentials (their homepage names no one).

- **Section rhythm.** Most sections were already at the 6–8rem desktop target (`py-24 md:py-32`); the real compression was the stats row having no dedicated section at all, and mobile sitting at the same value as one of the desktop breakpoints rather than scaling down from it. Added one shared token, `SECTION_PADDING` in `lib/utils.ts` (4rem mobile → 6rem md → 8rem lg), applied to every full content section. Deliberately not applied to ProofStrip (a thin trust bar by design) or Hero (the one section meant to carry more visual weight, per the competitor study's own first observation).
- **Services grid.** Already matched the target spec almost exactly — uniform outlined lucide icons, same size/weight/color, 3/2/1 responsive grid, name plus one line plus booking link. Confirmed rather than rebuilt.
- **Stats row extracted** into its own section (`Stats.tsx`) with a line icon per figure, matching the service icon set's weight and style. Same four figures, no numbers invented. Removed the now-duplicate "5,000+ patients served" line from `ProofStrip.tsx` — once Stats had its own generous section restating the number, leaving it in both places read as assembled rather than designed, exactly what this pass was against. ProofStrip wasn't named in the brief; flagged before doing it.
- **Hero restructure.** The two ambient background circles removed; one mint circle repurposed into an image container behind the navy feature card, which now overlaps it (`ml-6 mt-20` pulling the card up and left into the shape, `z-10` keeping it on top) rather than sitting beside it. No asset sourced or generated, per instruction — `heroSubjectImageSrc` is an explicit `undefined` at the top of `Hero.tsx` with a comment marking exactly where a real path goes; absent an image the circle renders as a plain soft-mint fill, reading as an intentional background form rather than a placeholder box, since nothing about the markup visibly signals "missing image" to a visitor. Three small lucide `Sparkle`/`Star` accents added, inline SVG via the existing icon library rather than sourced/hand-drawn assets. Right-column composition stays `hidden lg:block`, unchanged existing behavior, not something this pass was asked to touch.
- **AI assistant merchandised.** `AIDemo.tsx` moved to immediately follow Hero (was four sections down), copy converted fully to Indonesian (was mixed — Indonesian sample questions under an English headline), chips wired to actually send the clicked question rather than just opening the panel. Needed real plumbing: `ChatWidget.tsx` (`src/components/chat/`) is now a context provider wrapping the whole app (`layout.tsx` wraps `children` in it, not a sibling), exposing `useChatWidget()` with `open()` and `openWithMessage(text)`. `ChatPanel.tsx` accepts an `initialMessage` prop and sends it once via a ref-guarded effect, on mount, through the same `sendMessage()` path `ChatEmptyState`'s own chips already use. Same chat implementation, no second one. Verified live (mocked `/api/chat`): clicking a marketing-page chip opens the panel with the question already sent, not sitting in the empty state waiting for a second click.
- **Pricing and credentials surfaced earlier.** A short line under Hero's CTAs points at `#pricing` — written in English to match the rest of Hero, since the brief's own Indonesian example phrase would have violated its own no-mixed-language-within-a-section rule; flagged rather than resolved silently. `Doctors.tsx` pulls years of practice and university out of prose into a labeled strip per doctor, and adds a distinct `Sp.KG`/`Sp.Ort` badge next to the name — conditionally, since drg. Citra Dewi genuinely has no Sp. title (RUN A) and nothing here invents one for her. Budi's and Citra's universities (Universitas Airlangga, Universitas Trisakti) were already fact-checked into this document's own doctor reference table earlier but had never made it onto the page itself; added now, not invented.
- **Two pre-existing em dashes fixed in passing**, both in `Doctors.tsx` alt text, since the file's data array was already being rewritten for the credential fields.
- **Verified, not just built.** Screenshots at 375px and 1280px for every changed section, actually viewed, not just confirmed to exist. Caught one real thing this way: at 375px, the floating chat launcher (`fixed bottom-6 right-6`) overlaps Citra Dewi's specialty text at a specific scroll position — confirmed with a real single-viewport screenshot at that exact scroll position, not assumed from a stitched full-page capture (a stitched capture also showed an unrelated sticky-nav duplication artifact, which is not a real bug — screenshot tooling, not layout, learned earlier this session and reconfirmed here). The overlap is inherent to any fixed floating action button over scrollable content and predates this pass; the taller doctor cards from the credential work shifted where it lands but didn't create the underlying behavior. Not fixed, since repositioning the global chat launcher is outside a spacing/hierarchy pass. `tsc`, `eslint`, Vitest (29/29), Playwright (22/22, chromium) all clean. Lighthouse: accessibility held at 90 across two runs; performance read 95 on a first run and 97 on a re-run with nothing changed between them, confirming the first number was measurement noise in this sandbox rather than a regression, not just assumed.
Branch: `feat/marketing-ui-pass` · **merged** (PR #31).

**Floating chat launcher, mobile overlap fixed.** The item flagged and deliberately left unfixed above (`fixed bottom-6 right-6` landing on top of doctor-card text at some scroll positions) got a real fix rather than staying documented debt: the launcher shrinks from 56px to 48px below the `sm` breakpoint (`h-12 w-12` mobile, `h-14 w-14` from `sm` up, insets tightened to match), and `ChatWidget.tsx` now tracks scroll activity (a `scroll` listener plus a 350ms idle timeout) and fades the launcher to 40% opacity while the page is actively moving, back to full opacity once it settles — off while the panel itself is open, so it doesn't fade mid-conversation. This doesn't claim to make a fixed corner element never touch any text at any scroll position (no fixed FAB over scrollable content can guarantee that without abandoning the pattern), but it materially reduces both the footprint and the fraction of scroll time spent fully opaque over content. Verified: a real 375px browser check confirmed the 48px sizing and a genuine opacity-40 → opacity-100 class transition tied to scroll state (not just code review), plus the full Playwright suite (22/22, chromium) including the two specs that assert on `button[aria-label="Open chat"]`'s position relative to the open panel. `tsc` and `eslint` clean.
Branch: `fix/chat-launcher-mobile-overlap` · pending PR.

**Supabase advisor cleanup — RLS performance, missing FK indexes, redundant leads policy.** Ran `get_advisors` (performance) against the live project rather than assuming what needed fixing; all three findings below matched what was reported and were verified against the actual `pg_policies`/`pg_indexes` state before writing the migration, not guessed from the advisor text alone.

- **`auth_rls_initplan`.** All six `"Admin full access to X"` policies (`documents`, `document_chunks`, `embeddings`, `conversations`, `messages`, `leads`) called `auth.role() = 'authenticated'` directly in `USING`, re-evaluated per row. Wrapped in `(select auth.role())` on all six so Postgres treats it as an InitPlan, evaluated once per statement — no access change, cheaper at scale.
- **`unindexed_foreign_keys`.** `conversations.lead_id`, `document_chunks.document_id`, `embeddings.chunk_id`, `messages.conversation_id` had no covering index — confirmed via `pg_indexes`, each only had its primary key. Every cascade delete along `documents → document_chunks → embeddings` and `conversations → messages`, and every join on these columns, was a sequential scan of the child table. Added one index per column.
- **`multiple_permissive_policies` on `leads`.** `"Admin full access to leads"` was `FOR ALL`, so it already covered `INSERT` on top of the separate `"Public insert leads"` policy — both permissive, both evaluated on every anon/authenticated insert. Postgres can't express "ALL except INSERT" in one `CREATE POLICY`, so the `ALL` policy was dropped and replaced with three single-command policies (`Admin select/update/delete leads`, same InitPlan fix), leaving `"Public insert leads"` as the sole policy governing `INSERT`.
- **Verified, not just applied.** Ran the migration live via `apply_migration` against the same project, then re-ran `get_advisors` — all three finding types gone, only `unused_index` INFO notices remained on the four new indexes (expected, they have zero query history yet). Confirmed the leads insert path still works for `anon`: `SET ROLE anon; INSERT ... RETURNING id` failed on RLS, but that's `INSERT ... RETURNING` requiring SELECT-visibility of the row, unrelated to this migration and true before it too (the real app never uses that path — `/api/leads` writes via `createServiceClient()`, which bypasses RLS); the same insert without `RETURNING` succeeded. Smoke-test rows deleted afterward. `get_advisors` (security) re-checked too — nothing new, the same four pre-existing findings from earlier in this document (documented `SECURITY DEFINER` functions, `vector` extension in `public`, leaked password protection) are unchanged.
- **Not fixed here, flagged instead:** Supabase Auth's leaked-password-protection setting is still disabled — it's a dashboard toggle (Auth → Policies → Password), not something a SQL migration can set, so it needs Yoga to enable it manually for admin login security.

Branch: `chore/supabase-rls-perf-hardening` · pending PR.
...
6. **`SUPABASE_SERVICE_ROLE_KEY` has no Preview scope in Vercel** — Production only. `/api/leads` and `/api/knowledge` both call `createServiceClient()`, so any Preview deployment hitting either route throws `supabaseKey is required.`
