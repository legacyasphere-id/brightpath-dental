...
⚠️ **Pending live verification.** Once Yoga deletes the override and redeploys: re-ask "ada layanan apa saja?" and confirm `sources` is non-empty and includes `01-services.txt`, and the answer names dental implants and pediatric dentistry without naming "Perawatan periodontal." Then run one negative test — a question the KB genuinely doesn't cover — to confirm the zero-chunk guard fires correctly once retrieval is healthy, not just on the broken retrieval path it was first observed on. The guard currently has unit tests plus exactly one production observation, and that observation came from a retrieval path that was itself broken; refusing correctly with healthy retrieval is a different behavior and hasn't been tested yet.
