...
Chosen over a HyDE-style approach (ask the model to draft a hypothetical answer, embed that) specifically because this KB is six documents about one clinic — doesn't need per-question cleverness — and a streaming chat reply can't absorb an extra model round trip for free. Flagged going in, not yet re-measured: the winning long-query experiment wasn't just *longer*, it happened to echo the target document's actual vocabulary ("durasi", "prosedurnya" appear throughout `01-services.txt`'s structure). A generic template's broad vocabulary net is designed to capture that overlap without being hand-tuned, but whether it's *enough* — as opposed to just reducing diffuseness generally without differentially favoring `01-services.txt` — is an empirical question, not yet answered, since this sandbox has no live OpenRouter access to test it.

⚠️ **Pending live verification.** Ask "ada layanan apa saja?" (and 2–3 other short paraphrases, not just this one) in production and read `sources` back. Success: `01-services.txt` appears and ranks competitively, not just barely clears the floor. If it doesn't, or only does inconsistently across paraphrases, that's the signal to build the HyDE version instead of tuning the template further.
...
**Final verification pass — nothing regressed.** `tsc --noEmit` clean, `eslint` clean, `npm run build` zero type errors, Vitest 29/29, Playwright 22/22 (chromium). Manual check at 375px: hero, all nine service cards, doctor row, pricing, and the chat panel — screenshotted directly (`page.locator(...).screenshot()` per section, not a full-page capture, so nothing was hidden below an unscrolled fold), all render cleanly, nothing clipped. Lighthouse against the local production build: performance 97, accessibility 90, best-practices 100, SEO 100.

Three accessibility findings from Lighthouse, all **pre-existing, not introduced by anything in today's session** — confirmed by checking which component each belongs to before reporting, not assumed:
- `select-name` (weight 10): `LeadForm.tsx`'s service `<select>` has a visible "Service Needed" `<label>` above it, but the label isn't wired to the select via `htmlFor`/`id`, so screen readers don't associate them. Only the `services` array was touched this session (three new options added) — the label/select markup itself predates this run.
- `color-contrast` (weight 7, 26 instances): widespread, spans nearly every section (footer, pricing, doctors, AI demo, lead form) — a site-wide pattern of muted-text utility classes (`text-clinic-muted`, `text-white/60` on the navy pricing card) not meeting 4.5:1, not anything specific to this session's changes.
- `heading-order` (weight 3): one `<h4>` in `Footer.tsx` that skips a level — `Services.tsx`'s new cards use `<h3>`, not `<h4>`, so this isn't related to today's additions either.

Not fixed here — flagged since they predate this session and weren't in scope for a verification pass, not because they don't matter.
